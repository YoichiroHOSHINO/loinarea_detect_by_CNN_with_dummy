...
X = np.load(path + 'LoinDummy_mix_img_for_train.npy')  
# 学習用座標データセットのパスを指定してください。
Y = np.load(path + 'LoinDummy_mix_label_for_train.npy')

# 任意の試験名を指定してください。出力結果はすべてこの名前＋αが付けられます。
testname = 'LoinDummy_mix_train_201223'
...
x = Flatten()(x)
x = Dense(5000, activation='relu', name='dense_relu_1')(x)
#x = Dropout(0.05)(x)
